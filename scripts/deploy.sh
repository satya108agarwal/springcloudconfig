...
{
  cf create-service -c '{ "git": { "uri": "https://github.com/satya108agarwal/spring-cloud-config-prop", "label": "main"  } }' ${service_name} standard my-config-server
} &>/dev/null
until [ "$(cf service my-config-server | grep -c "succeeded")" -eq 1 ]; do
  echo -n "."
done
